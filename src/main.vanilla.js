...
class Utils {
  ...
  static lightenHex(hex, amount = 0.2) {
    // Adapted from common lightening algorithms
...
class StreakCalculator {
  ...
  async calculate(habit) {
    ...
    for (let i = 0; i <= MAX_LOOKBACK_DAYS && fileReads < MAX_FILE_READS; i++) {
    ...
    longestStreak = Math.max(longestStreak, habit.savedLongestStreak || 0);
    ...
    const result = { currentStreak, longestStreak, firstCompletionDate, consistencyScore, consistencyLabel, consistencyCompleted, consistencyScheduled, recoveryScore, ongoingGapLength };
